...
class search_by_triplet_trie:
  """Solver based on creating a trie of 
  all possible combinations in all triplets."""
  def __init__(self):
    # Constants
    self.max_scatter = 0.1

  # Merge modules two by two
  def merge_module_pairs(self, ev):
    module_pairs = []
  ...
  def generate_compatible_triplets(self, module_pairs):
    ...
    for m0, m1 in zip(reversed(module_pairs[2:]), reversed(module_pairs[1:-1])):
      ...
      compatible_triplets += self.check_best_triplets(m0, m1, module_pairs[m2])
  ...
  def solve(self, event):
    ...
    module_pairs = self.merge_module_pairs(event)

    # Build a trie with compatible triplets per module and hit pair
    compatible_triplets_trie = self.generate_compatible_triplets(module_pairs)
    ...
    for m0, m1 in zip(reversed(module_pairs[2::1]), reversed(module_pairs[:-2:1])):
      ...
      for t in forwarding_tracks:
        ...
        if t.missed_last_module or t.missed_penultimate_module:
          ...
          best_h2 = hit(0, 0, 0, -1)
          best_scatter = self.max_scatter
          hits_m2 = event.modules[m1.module_number - 1].hits()
          for h2 in hits_m2:
            scatter = self.calculate_scatter(h0, h1, h2)
